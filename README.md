...
フォルダごとに整理して、あとから**構造化検索 + 意味検索**で探せるようにするツールです。
...
1. **取り込み** 監視フォルダの `_inbox` に入れる、または(デスクトップ版)**フォルダ自動監視**
...
6. **整理** 「フォルダ → 種別 → ファイル」階層 + 構造化フィルタ
7. **検索** 構造化(種別・日付)+ 埋め込みの意味検索(端末内でコサイン類似度を計算)
...
`http://localhost:3000` を開き、「整理」でワーキングディレクトリを指定して `_inbox` にファイルを入れます(ログイン不要)。
...
> 各サブフォルダに振り分けられたファイルは、検索・カレンダー・ドラフト生成など既存機能から扱えます。
